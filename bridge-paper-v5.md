...
The atlas is derived from **74 reviewed academic works across 22 research domains, with 516 structured evidence extractions** and **735 mechanism-evidence links**, each quality-graded and linked to architectural elements. **59 foundational researchers** are mapped. **14 cross-tradition convergences** have been formally identified, points where independent research traditions, using incompatible methods and frameworks, arrive at the same structural conclusion about the architecture. **15 evolved mechanisms** are derived from the convergences, each representing a functional system the architecture maintains. The convergences are versioned, status-tracked, and adversarially reviewed.
...
DA8 is the foundation that most directly undermines the optimizer-era assumption that humans can simply *decide* not to be affected by what the optimizer surfaces. They cannot. The architecture is not built that way. Cortical override has real metabolic cost, and chronic defensive activation actively suppresses the cortical machinery that would override. DC3 states the consequence as a foundation in its own right: environment is the primary intervention layer for systemic design, not because user-side interventions fail, but because override capacity is degraded by exactly the chronic activation states that most demand it.
...
DA8, phylogenetic priority, is what makes all six failure modes resistant to user-side cognitive override. Telling a user to "just put down the phone" is asking the cortex to override systems with denser projections in the opposite direction. Chronic defensive activation actively suppresses the override machinery. The empirical demonstrations of environment correction (Section 6) show that environmental correction is frequently more durable and less effort-dependent than internal reframing alone (DC3). For optimizer-tier systems, this means the meaningful design target is the environmental layer itself. Asking users to opt out is rarely the right systemic remedy, and at population scale it is the wrong layer for institutional design.
